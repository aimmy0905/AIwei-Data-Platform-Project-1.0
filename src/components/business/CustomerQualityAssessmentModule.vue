<template>
  <div class="customer-quality-assessment-module">
    <div class="analysis-module__header">
      <h3 class="analysis-module__title">客户质量评估（总计）</h3>
    ...
    <div class="analysis-module__content">
      ...
      <div class="quality-table-container">
        <table class="quality-table">
          ...
          <tbody>
            <tr v-for="qualityData in customerQualityData" :key="`${qualityData.year}-${qualityData.quarter}`" class="data-row">
              <td class="time-cell" v-if="qualityData.isFirstOfYear" :rowspan="qualityData.quarterCount">{{ qualityData.year }}</td>
              <td class="time-cell">{{ qualityData.quarter }}</td>

              <!-- 月消费 10万美金以上客户 -->
              <td class="data-cell high-value">{{ qualityData.highValue.customers }}</td>
              <td class="data-cell high-value">{{ formatCurrency(qualityData.highValue.sales) }}</td>
              <td class="data-cell high-value">{{ formatCurrency(qualityData.highValue.consumption) }}</td>
              <td class="data-cell high-value" :class="getRoiClass(qualityData.highValue.roi)">
                {{ formatPercentage(qualityData.highValue.roi) }}%
              </td>

              <!-- 月消费 5-10万美金客户 -->
              <td class="data-cell medium-high">{{ qualityData.mediumHigh.customers }}</td>
              <td class="data-cell medium-high">{{ formatCurrency(qualityData.mediumHigh.sales) }}</td>
              <td class="data-cell medium-high">{{ formatCurrency(qualityData.mediumHigh.consumption) }}</td>
              <td class="data-cell medium-high" :class="getRoiClass(qualityData.mediumHigh.roi)">
                {{ formatPercentage(qualityData.mediumHigh.roi) }}%
              </td>

              <!-- 月消费 3-5万美金客户 -->
              <td class="data-cell medium">{{ qualityData.medium.customers }}</td>
              <td class="data-cell medium">{{ formatCurrency(qualityData.medium.sales) }}</td>
              <td class="data-cell medium">{{ formatCurrency(qualityData.medium.consumption) }}</td>
              <td class="data-cell medium" :class="getRoiClass(qualityData.medium.roi)">
                {{ formatPercentage(qualityData.medium.roi) }}%
              </td>

              <!-- 月消费 1.5-3万美金客户 -->
              <td class="data-cell medium-low">{{ qualityData.mediumLow.customers }}</td>
              <td class="data-cell medium-low">{{ formatCurrency(qualityData.mediumLow.sales) }}</td>
              <td class="data-cell medium-low">{{ formatCurrency(qualityData.mediumLow.consumption) }}</td>
              <td class="data-cell medium-low" :class="getRoiClass(qualityData.mediumLow.roi)">
                {{ formatPercentage(qualityData.mediumLow.roi) }}%
              </td>

              <!-- 月消费 1.5万以下美金客户 -->
              <td class="data-cell low-value">{{ qualityData.lowValue.customers }}</td>
              <td class="data-cell low-value">{{ formatCurrency(qualityData.lowValue.sales) }}</td>
              <td class="data-cell low-value">{{ formatCurrency(qualityData.lowValue.consumption) }}</td>
              <td class="data-cell low-value" :class="getRoiClass(qualityData.lowValue.roi)">
                {{ formatPercentage(qualityData.lowValue.roi) }}%
              </td>
            </tr>
          </tbody>
        </table>
...
interface CustomerQualityData {
  ...
  lowValue: CustomerSegment      // 月消费 1.5万以下美金客户
}
...
// 客户质量评估数据
const customerQualityData = ref<CustomerQualityData[]>([
  {
    year: '2025年',
    quarter: '2025年',
    isFirstOfYear: true,
...
const getRoiClass = (roi: number) => {
  if (roi >= 150) return 'high-roi'
  if (roi >= 100) return 'medium-roi'
  return 'low-roi'
}
</script>

<style scoped>
.customer-quality-assessment-module {
  background: #fff;
  border-radius: 8px;
...
.data-cell.roi-metrics.high-roi {
  color: #52c41a !important;
  font-weight: 600;
}

.data-cell.roi-metrics.medium-roi {
  color: #faad14 !important;
  font-weight: 600;
}

.data-cell.roi-metrics.low-roi {
  color: #ff4d4f !important;
  font-weight: 600;
}
